...
import { products, uploadProducts } from "../service/uploadProducts";
...
export const Header = () => {
  ...
  const handleLoginLogout = () => {
    if (user?.isLoggedIn) {
      ...
      login({ name: "Gonzalo Galvan", isLoggedIn: true });
  ...
  const cargarProducts = async () => {
    try {
      const productRef = collection(db, "Cafes"); // referencia a la coleccion
  ...
  const uploadProductos = async () => {
    try {
      const productRef = collection(db, "Cafes"); // Referencia a la colección
      const snapshot = await getDocs(productRef);

      // Actualiza cada documento con el campo "stock"
      snapshot.forEach(async (docSnap) => {
        const docRef = doc(db, "Cafes", docSnap.id); // Obtiene referencia al documento
  ...
  const uploadBatchProductos = async () => {
    try {
      ...
      const productRef = collection(db, "Cafes");
      const snapshot = await getDocs(productRef);
  ...
  const deleteProductById = async (id) => {
    try {
      const docRef = doc(db, "Cafes", id); // Referencia al documento por ID
  ...
  const deleteAllProducts = async () => {
    try {
      ...
      const productsRef = collection(db, "Cafes"); // Referencia a la colección
  ...
  const getFilteredProducts = async () => {
    try {
      const productsRef = collection(db, "Cafes");
      const q = query(
        productsRef,
